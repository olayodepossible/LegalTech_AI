...
async def run_research_agent(topic: str = None) -> str:
    ...
    # Bedrock region and model (override with BEDROCK_REGION / BEDROCK_MODEL_ID in App Runner / .env)
    ...
    # NOTE: nova-pro is needed to support tools and MCP servers; nova-lite is not enough
    model_id = os.getenv("BEDROCK_MODEL_ID", "eu.amazon.nova-pro-v1:0")
    model = LitellmModel("openrouter/openai/gpt-4o-mini", api_key=os.getenv("OPENROUTER_API_KEY"))

    # Create and run the agent with MCP server
    with trace("Researcher"):
        async with create_playwright_mcp_server(timeout_seconds=60) as playwright_mcp:
            agent = Agent(
                name="FinPlex Investment Researcher",
...
async def health():
    ...
    return {
        "service": "FinPlex Researcher",
        "status": "healthy",
        "finplex_api_configured": bool(os.getenv("FINPLEX_API_ENDPOINT") and os.getenv("FINPLEX_API_KEY")),
        "timestamp": datetime.now(UTC).isoformat(),
