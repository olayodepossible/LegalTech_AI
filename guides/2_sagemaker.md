...
aws sagemaker-runtime invoke-endpoint --endpoint-name legal-companion-embedding-embedding-endpoint --content-type application/json --body fileb://vectorize_me.json --output json /dev/stdout
```
...
# Invoke the endpoint and output directly to console
aws sagemaker-runtime invoke-endpoint --endpoint-name "legal-companion-embedding-endpoint" --content-type "application/json" --body "fileb://vectorize_me.json" --output json output.json
...
```bash
aws sagemaker describe-endpoint --endpoint-name legal-companion-embedding-endpoint
```
...
1. **Verify the HuggingFace model ID**:

Check that the endpoint is configured with the correct model:

```bash
aws sagemaker describe-model --model-name legal-companion-embedding-model --query 'PrimaryContainer.Environment'
```
...
- Language usage evolves (new data terms emerge)
...
3. **Check Your Endpoint**:
  ...
  - Find `legal-companion-embedding-endpoint`
...
Let's clarify when to use each service:
...
| **Best For**         | • Custom models • Fine-tuned models • Specialized embeddings • Full ML pipelines | • General language tasks • Quick prototypes • Standard AI capabilities |
...
```bash
terraform import aws_sagemaker_endpoint.embedding_endpoint legal-companion-embedding-endpoint
terraform apply
```
...
```bash
aws sagemaker delete-endpoint --endpoint-name legal-companion-embedding-endpoint
