# Ingestion Pipeline with S3 Vectors

 In this guide, we'll deploy a cost-effective vector storage solution using AWS S3 Vectors:
...
4. Configure:
  - Bucket name: `legal-vectors-{your-account-id}` (replace with your actual account ID)
  - Encryption: Keep default (SSE-S3)
5. After creating the bucket, create an index:
  - Index name: `legal-research`
  - Dimension: `384`
...
sagemaker_endpoint_name = "legal-companion-embedding-endpoint"  # From Part 2
```
...
VECTOR_BUCKET=legal-vectors-YOUR_ACCOUNT_ID
...
graph LR
    ...
    style E fill:#084508,stroke:#228B22,stroke-width:3px
```
...
- Check CloudWatch logs: `aws logs tail /aws/lambda/legal-ingest`
