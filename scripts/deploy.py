#!/usr/bin/env python3
"""
Deploy the Legal Companion Advisor infrastructure.
This script:
...
def check_prerequisites():
    ...
    for tool, message in tools.items():
        if tool == "npm":
            ...
            version_cmd = [npm_exe, "--version"] if npm_exe else None
        else:
        ...
        try:
            ...
            run_command(version_cmd, capture_output=True)
...
def build_frontend(api_url=None):
    ...
    npm_exe = resolve_npm_executable()
    if not npm_exe:
        print("  ❌ npm not found. Install Node.js or fix PATH.")
        sys.exit(1)
...
def main():
    ...
    print("🚀 Legal Companion Advisor -  Deployment")
    print("=" * 50)
